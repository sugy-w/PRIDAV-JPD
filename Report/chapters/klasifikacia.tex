...
\begin{itemize}
    ...
    a potom previedli na binárne vektory pomocou \textit{to\_categorical} z knižnice \textit{keras}.
\end{itemize}
...
To nám ukázalo problém nevývaženosti dát, v matici nie je takmer nič na diagonále - tzn. veľmi nízku presnosť.
...
Z tejto vizualizácie možno model hodnotiť pozitívne, pretože `svetlé' štvorčeky sú prevažne na diagonále, čo znamená lepšiu presnosť.
...
\textbf{Noc} má zanedbateľný vplyv na klasifikáciu (nízky koeficient). Keďže v noci je aktivita minimálna na všetkých trasách rovnako, tento časový úsek nenesie žiadnu rozlišovaciu informáciu.
\textbf{Ráno (Po – Str, Ned)}: Ranná špička je kľúčovým identifikátorom najmä v prvej polovici pracovného týždňa a prekvapivo aj v nedeľu (športová aktivita).
\textbf{Obed (Pia, So)}: S blížiacim sa víkendom sa ťažisko dôležitosti presúva. V piatok a sobotu je pre rozlíšenie trasy najpodstatnejšia aktivita okolo obeda.
\textbf{Popoludnie} si zachováva relatívne vysokú dôležitosť počas celého týždňa, čo značí, že poobedné návraty z práce alebo výletov sú silným signálom pre model.
Počas \textbf{víkendových večerov} dôležitosť opäť klesá.
